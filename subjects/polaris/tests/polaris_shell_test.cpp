...
class PolarisShellTest final : public QObject {
    Q_OBJECT
...
void PolarisShellTest::secondInstanceDefersToTheFirst() {
    // 同一台机器上可能并行跑着别的测试，key 带上 pid 才不互相抢。
    const QString key =
        QStringLiteral("athena.polaris.test.%1").arg(QCoreApplication::applicationPid());
...
void PolarisShellTest::reclaimsKeyLeftBehindByACrash() {
    const QString key =
        QStringLiteral("athena.polaris.stale.%1").arg(QCoreApplication::applicationPid());
    {
...
QTEST_GUILESS_MAIN(PolarisShellTest)
#include "polaris_shell_test.moc"
