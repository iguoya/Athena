...
class PolarisCatalogTest final : public QObject {
    Q_OBJECT
...
// 学科入口上，不能落在方向图或目标图上。
void PolarisCatalogTest::defaultsToAnAcademicEntryMap() {
    PolarisCatalog catalog(QString::fromUtf8(POLARIS_SOURCE_ROOT));
    QCOMPARE(catalog.selectedMapId(), QStringLiteral("computer-science"));
...
void PolarisCatalogTest::bindsNecessityToCareerTargets() {
    PolarisCatalog catalog(QString::fromUtf8(POLARIS_SOURCE_ROOT));
...
void PolarisCatalogTest::keepsCrossMapDependenciesReachable() {
    PolarisCatalog catalog(QString::fromUtf8(POLARIS_SOURCE_ROOT));
    // C 语言在程序设计图、51 单片机在单片机图，这条强先修必须还在。
    const QVariantList links = catalog.crossEdgesFor(QStringLiteral("polaris.ei.mcu_8051"));
    ...
    for (const QVariant& value : links) {
        ...
        if (link.value("peer_id").toString() == QStringLiteral("polaris.cs.c_lang")) {
            foundCLanguage = true;
...
void PolarisCatalogTest::preservesStrictMapSelection() {
    PolarisCatalog catalog(QString::fromUtf8(POLARIS_SOURCE_ROOT));
    catalog.openMap("embedded-realtime");
    QCOMPARE(catalog.selectedMapId(), QStringLiteral("embedded-realtime"));
    catalog.selectNode("polaris.embedded.bringup");
    QCOMPARE(catalog.selectedNode().value("id").toString(), QStringLiteral("polaris.embedded.bringup"));
...
void PolarisCatalogTest::restoresCppKnowledgeGraphAsTwoChapters() {
    PolarisCatalog catalog(QString::fromUtf8(POLARIS_SOURCE_ROOT));
    QVERIFY2(catalog.error().isEmpty(), qPrintable(catalog.error()));
    ...
    QVERIFY(catalog.edges().size() >= 13);
    catalog.selectNode("polaris.cs.cpp");
    ...
    for (const QVariant& value : catalog.edges()) {
        ...
        if (edge.value("from").toString() == QStringLiteral("polaris.cs.c_lang")
            && edge.value("to").toString() == QStringLiteral("polaris.cs.cpp")) {
    ...
    catalog.selectNode("polaris.ei.electronics_basics");
    QCOMPARE(catalog.selectedNode().value("entry").toBool(), true);
    QCOMPARE(catalog.selectedNode().value("verify").toString(), QStringLiteral("bench"));
    catalog.selectNode("polaris.ei.analog");
    QCOMPARE(catalog.selectedNode().value("id").toString(), QStringLiteral("polaris.ei.analog"));
    catalog.selectNode("polaris.ei.digital");
    QCOMPARE(catalog.selectedNode().value("id").toString(), QStringLiteral("polaris.ei.digital"));
}

// 职业图仍在内容里（宁增勿删），但当前盘面只开放技术体系（ADR 0012）。
void PolarisCatalogTest::locksCareerMapsUntilKnowledgeSystemIsComplete() {
    PolarisCatalog catalog(QString::fromUtf8(POLARIS_SOURCE_ROOT));
...
void PolarisCatalogTest::courseChaptersCarryNecessityAndPractice() {
    PolarisCatalog catalog(QString::fromUtf8(POLARIS_SOURCE_ROOT));
    QVERIFY2(catalog.error().isEmpty(), qPrintable(catalog.error()));
    catalog.openMap("computer-science");
    for (const QVariant& nodeValue : catalog.nodes()) {
...
QTEST_MAIN(PolarisCatalogTest)
#include "polaris_catalog_test.moc"
