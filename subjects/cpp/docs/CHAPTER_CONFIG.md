...
五部分都要出现且不得为空。末尾的回顾可以是「小结」，也可以是误区清单这类
本来就在收尾的内容——职责落实即可，不必为凑标题另加一节（见 AGENTS.md）。按
...
`labs` 给这个知识点挂**可编辑的骨架案例**（ADR 0053）：学员在专注实验页的
「动手实验」标签里改代码、用本机编译器跑，和只读的 `source` 实验并存——那条给
「看懂它长什么样」，这条给「自己写一遍」。可以不挂，挂多条时目前只用第一条。
...
```json
{
  ...
  "labs": [
    {
      "case": "initialization_forms",
      "prompt": "同样带一个等号，Tracer copy = a; 和 Tracer b = 2; 是同一回事吗？",
      "goal": "补出 todo_copy_init() 里的那一行，对照两段输出里构造函数的调用。",
      "hint": "先写下你的预测再跑。"
    }
  ]
}
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---:|---|
| `case` | string | 是 | 案例目录名，对应 `resources/cases/<case>/`；只允许 `[a-z0-9_]` |
| `prompt` | string | 是 | **题干**：要验证或解决什么、观察什么现象。读者第一眼读这个 |
| `goal` | string | 是 | **动手清单**：补哪个符号、对照哪段输出 |
| `hint` | string | 否 | 提示；留空则界面不显示这一行 |

`prompt` 和 `goal` 都必填，是因为只写「补全 xxx」而看不到认知问题，正是
`subjects/dsa` ADR 0003 第 5 条点名要避免的写法。

**案例骨架的验收标准：不改任何一行就能编译运行。** 骨架要自带必要头文件、
待实现的接口（可带空占位体）和固定的 `main` 驱动，并用注释划清可改区域
（`// TODO(学员)：…` 与 `// —— 以下驱动请勿改 ——`）。示范与待填可以并存：
先给一个已实现的参考函数供阅读，再留一个同构的待填变式。

生成器会校验目录存在、非空、至少有一个带 `int main` 的源文件、`case` 名合法、
同一知识点内不重复。案例文件由 GResource 打包（`/app/cases/<case>/<文件>`），
运行期展开成用户数据目录下的工作副本供编辑；仓库里的原件任何时候都不被应用
改写，界面上的「重置骨架」就是删掉副本重新展开。
