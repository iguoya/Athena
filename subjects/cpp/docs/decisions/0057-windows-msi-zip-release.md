...
本应用已在 Windows（MSYS2 UCRT64）上构建并跑过 CI，但标签 Release 只产
...
- `scripts/package_windows.py` 是 Windows 发行包的唯一入口：从一份
  `athena-cpp.exe` 生成 `athena-cpp-VERSION-windows-x64.zip` 与
  `athena-cpp-VERSION-windows-x64.msi`。
- 包维持 MSYS2 的 `bin` / `lib` / `share` 布局。glib 在 Windows 上按
  自身 DLL 所在目录的上一级当 prefix，这套布局换机即可运行，不要求再装
  MSYS2。
- zip 是便携下载；MSI 用 WiX 5 整树收进 `Program Files\athena-cpp`，并放一条
  开始菜单快捷方式（显示名「C++ 教程」，入口 `athena-cpp.cmd`）。两者内容相同。
  当前不做 Authenticode 签名。UpgradeCode 沿用，升级会卸掉旧的 Athena 安装项。
...
- 修改 Windows 打包器或发行工作流后，应在本机生成 zip，并至少启动一次
  `athena-cpp.cmd`；MSI 由 GitHub Windows Runner 用固定版本的 WiX 打出。
