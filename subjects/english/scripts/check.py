#!/usr/bin/env python3
"""英语学习应用的验证入口：内容校验、前端构建、Rust 侧类型检查。
...
def main() -> int:
    _force_utf8_output()
    parser = argparse.ArgumentParser(description="验证英语学习应用")
    parser.add_argument(
        "--skip-rust",
        action="store_true",
    ...
    check_json()
    ensure_dependencies()
    # npm run build = check:content + tsc + vite build：内容规则、类型和
    # 打包一次过。内容检查留在 npm 那边，本地 npm run build 也会跑到它。
    ...
    if not arguments.skip_rust:
        ...
        # 跑测试，不只是编译。subjects/cpp 在 Windows 上那个 CRLF 问题就是测试逮到的
