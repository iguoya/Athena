...
与 C++ 教程**平级**的独立学习应用（ADR 0032）。技术栈自选：Tauri 2 + Web UI +
本机 C++ 工具链。从启动器打开；**不启动别的应用也可以完成全部学习业务**。
...
4. **谁都能拉起它**：启动器和终端走的都是本目录 `app.json` 的 dev 声明；进度写在本应用自己的库里。
...
DSA 的课程和实验仍只认 `content/`。如需核对某个案例假定的 C++ 语言能力，阅读
[`docs/cpp-foundations-reference.md`](docs/cpp-foundations-reference.md)；该文档只提供
术语与边界参考，不引入对 `subjects/cpp` 的构建、源码或配置依赖。
