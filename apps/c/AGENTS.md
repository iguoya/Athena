...
本文档是 **`apps/c` 独立应用** 的项目级指令。本应用与 C++ 教程
（`apps/cpp`，GTK / Meson）**平级、可脱离**。改本应用时以本文为准。
...
先吸收 `apps/cpp` 已经做对的教学结构，再画 C 自己的图。C++ 分类页的知识图谱节点是
...
不引入 `apps/cpp` 的 Meson、gtkmm、Blueprint。也不用 Tauri。
