...
先吸收主程序已经做对的教学结构，再画 C 自己的图。C++ 分类页的知识图谱节点是
**章**，不是知识点。点进一章之后，大纲里才有该章的知识点路线图；教案从那张图
点进来。

| 层 | 主程序 C++ | 本应用 |
|---|---|---|
| 知识图谱 | 分类下的章节卡片 + `prerequisites` | 首页六章 + `prerequisites` |
| 教学大纲 | 章内第一标签：方向 + 知识点路线图 | 点章之后：同一件事 |
| 教学过程 | 其余标签 | 章内路线图点白卡片才进入 |
| 教学实验 | 课程类成员函数 | 实验台拉起 `playground/` |
| 随堂考核 | `CheckpointView`：多题、有解析 | `Checkpoint.qml` + `exercises.json` |

不抄 TypeSemantics 的节标题，也不把 C++ 的引用/RAII/模板再讲一遍。章序跟 Beej
教程卷，不按 C++ 课类比出「对象与字节 / 函数与地址 / 堆与寿命」。

C 自己再发挥的是内存条、地址箭头、playground 里真实的 C 程序。这些只让教材里
...
| `scripts/fetch-sources.sh` | 把公开教材拉到 `content/sources/reference/` |
...
4. **随堂考核和课后习题**写在 `exercises.json`，按知识点 ID 挂上。每题必须有
   `source_refs`（教材节号 + 必要时 `c23` 条款）。题干用中文改写该节事实，不整段抄
   NC-ND 正文。教材与 C23 冲突时以 C23 为准，两边都引用。对不上的题不要写。
