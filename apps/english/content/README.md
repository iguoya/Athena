...
初级已从流程样本扩为第一批可持续练习库：300 个词、121 道例句轨练习（含 60 道
英译汉和 30 道连续短文）及 30 道开放写作。一个轨道可以在 `curriculum.json` 里用
`decks` 顺序引用多个内容单元；运行时合并统计，但每轮只取短回合。中高级仍是样本规模，
不应把全应用总题数误报成三个等级都已完成。
