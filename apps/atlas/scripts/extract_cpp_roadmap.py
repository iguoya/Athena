#!/usr/bin/env python3
"""把 apps/cpp 首页学科路线图的全部信息逐字导出成 JSON。

为什么要有这个脚本：那张图的内容手工维护在 apps/cpp/registry/domain_graph.cc
的 C++ 初始化列表里，是 atlas 技术体系图的原料。原料一旦只存在于别的应用的
源码里，那个应用一改动、一删除，信息就没了；靠人手抄 28 个节点 × 四段文字
外加三十多条依赖理由，必然漏字、漏条。所以用解析器机械提取，提取结果进
版本库，吸收工作再从 JSON 出发（ADR 0055 之后另见 atlas ADR 0009）。
...
用法：
    python3 apps/atlas/scripts/extract_cpp_roadmap.py [--check]
...
REPO_ROOT = ATLAS_ROOT.parent.parent
SOURCE = REPO_ROOT / "apps" / "cpp" / "registry" / "domain_graph.cc"
TARGET = ATLAS_ROOT / "content" / "absorbed" / "cpp-roadmap.json"

# DomainSpec 的字段顺序，见 apps/cpp/registry/domain_graph.h。
SPEC_FIELDS = [
...
def extract() -> dict:
    ...
    return {
        ...
        "note": (
            "apps/cpp 首页学科路线图的逐字导出，是 atlas 技术体系图的原料。"
            "字段含义见 apps/cpp/registry/domain_graph.h；kind/verify/priority/"
            "track 由同名枚举转写成小写。不要手改这个文件——它由 "
            "apps/atlas/scripts/extract_cpp_roadmap.py 生成。"
        ),
        "nodes": nodes,
