...
只跑一次：读 content/atlas.json 与 content/absorbed/cpp-roadmap.json，写出新的
atlas.json。节点正文、练习、验收和必要程度全部沿用已吸收的字段，只调整归属、
...
def absorbed_id(raw_node: dict) -> str:
    prefix = "atlas.cs." if raw_node["side"] == "computer" else "atlas.ei."
...
def main() -> None:
    ...
    if any(entry["id"] == "computer-science" for entry in atlas["maps"]):
        print("两章课程图已经在 atlas.json 里，无需再合并。")
        return
    ...
    for edge in atlas.get("cross_edges", []):
        existing_edges[(edge["from"], edge["to"])] = edge
