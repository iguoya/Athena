...
def main() -> None:
    ...
    maps = {entry["id"]: entry for entry in atlas["maps"]}
    ...
    cs_nodes["atlas.cs.operating_systems"]["priority"] = "essential"
    cs_nodes["atlas.cs.operating_systems"]["priority_reason"] = (
        "操作系统是计科培养的专业核心：进程、虚拟内存和并发原语是「能设计实现计算机系统」"
        "这条培养目标的底座，缺了就不构成这个学科。"
    )
    cs_nodes["atlas.cs.computer_networks"]["priority"] = "essential"
    cs_nodes["atlas.cs.computer_networks"]["priority_reason"] = (
        "计算机网络是计科培养方案里的专业核心课，也是「能在互联环境里实现系统」"
        "这条培养目标的直接对应；缺了这一层，学科图就是残的。"
    )
    cs_nodes["atlas.cs.linux_sysprog"]["priority"] = "important"
    cs_nodes["atlas.cs.linux_sysprog"]["priority_reason"] = (
        "培养方案不一定单列这门课，但系统能力目标要落到进程、文件描述符和系统调用上；"
        "它支撑操作系统与网络编程的实现，本身不是另一门核心理论课。"
    )
    cs_nodes["atlas.cs.concurrency"]["priority"] = "important"
    cs_nodes["atlas.cs.concurrency"]["priority_reason"] = (
        "并发是系统能力的深化，操作系统课已经建立概念；单独一门用来把正确的多线程写出"
        "可验证的结果，对培养目标重要，但不是另一条专业核心课。"
    )

    # —— 电子信息：对照电路 / 模电 / 数电 / 信号 / 通信 / 微机 ——
    ei_nodes["atlas.ei.mcu_8051"]["priority"] = "optional"
    ei_nodes["atlas.ei.mcu_8051"]["priority_reason"] = (
        "培养方案里的微机与嵌入式实践，当代平台是 32 位 MCU；51 只作为更缓的入门台阶"
        "保留在图上，避免有人不知道这条老路，但不把它当成 STM32 的闸门。"
    )
    ei_nodes["atlas.ei.dsp"]["priority"] = "essential"
    ei_nodes["atlas.ei.dsp"]["priority_reason"] = (
        "数字信号处理是电子信息培养方案的专业核心，对应「信息获取与处理」这条培养目标；"
        "信号与系统给数学语言，这一门要把滤波和变换做成可验证的实现。"
    )
    ei_nodes["atlas.ei.freertos"]["priority"] = "important"
    ei_nodes["atlas.ei.freertos"]["priority_reason"] = (
        "嵌入式方向实现目标需要多任务与实时调度，但不是所有电子信息核心课都叫 RTOS；"
        "它撑起产品级固件，列在重要档，不从核心课名单里挤掉模电数电通信。"
    )

    unforced_pairs = {
        ("atlas.cs.cpp", "atlas.cs.da"),
        ("atlas.cs.cpp", "atlas.cs.dp"),
        ("atlas.cs.linux_sysprog", "atlas.cs.concurrency"),
        ("atlas.cs.linux_sysprog", "atlas.cs.computer_networks"),
        ("atlas.ei.mcu_8051", "atlas.ei.stm32"),
        ("atlas.ei.stm32", "atlas.ei.pcb_design"),
        ("atlas.ei.sensors", "atlas.ei.dsp"),
        ("atlas.ei.stm32", "atlas.ei.dsp"),
        ("atlas.ei.electronics_basics", "atlas.ei.fpga"),
    }
    removed = drop_edges(cs, unforced_pairs) + drop_edges(ei, unforced_pairs)
    why = {
        ("atlas.cs.cpp", "atlas.cs.da"): "算法思想与语言无关；C++ 是本平台实现课的教学载体，不是知识门槛。",
        ("atlas.cs.cpp", "atlas.cs.dp"): "模式讲的是耦合怎么拆，不绑死在 C++ 上；示例语言不是先修。",
        ("atlas.cs.linux_sysprog", "atlas.cs.concurrency"): "并发原语在 C++ 标准库和 RTOS 里都能练，不必先过 Linux 系统调用。",
        ("atlas.cs.linux_sysprog", "atlas.cs.computer_networks"): "协议本身可以独立学；抓包实现是网络编程那门课的事。",
        ("atlas.ei.mcu_8051", "atlas.ei.stm32"): "51 是 gentler 的入门台阶，不是 STM32 的知识闸门；工程上可以直接上 32 位芯片。",
        ("atlas.ei.stm32", "atlas.ei.pcb_design"): "画板需要的是电路知识，不需要先用熟某一颗 MCU。",
        ("atlas.ei.sensors", "atlas.ei.dsp"): "DSP 是通用方法，可以在仿真里学；干净的前端是应用场景，不是课程门槛。",
        ("atlas.ei.stm32", "atlas.ei.dsp"): "实时 DSP 可以在 PC 或 FPGA 上做；STM32 只是常见落地平台。",
        ("atlas.ei.electronics_basics", "atlas.ei.fpga"): "FPGA 真正的知识门槛是系统的数字电路，不是面包板上的入门电路；先修改挂到数电。",
    }
    for entry, prefix in ((cs, "atlas.cs."), (ei, "atlas.ei.")):
        notes = []
        for edge in removed:
    ...
    set_requires(cs_nodes, "atlas.cs.da", [])
    set_requires(cs_nodes, "atlas.cs.dp", [])
    set_requires(cs_nodes, "atlas.cs.concurrency", [])
    set_requires(cs_nodes, "atlas.cs.computer_networks", [])
    set_requires(ei_nodes, "atlas.ei.stm32", [])
    set_requires(ei_nodes, "atlas.ei.dsp", [])
    set_requires(ei_nodes, "atlas.ei.pcb_design", ["atlas.ei.electronics_basics"])

    if not any(node["id"] == "atlas.cs.compilers" for node in cs["nodes"]):
    ...
    ei_nodes = node_map(ei)
    set_requires(ei_nodes, "atlas.ei.fpga", ["atlas.ei.digital_circuits"])
...
CS_NEW_NODES = [
    ...
    {
        "id": "atlas.cs.scientific_computing",
        "title": "科学计算",
        "track": "engineering",
    ...
    {
        "id": "atlas.cs.software_engineering",
        "title": "软件工程",
        "track": "engineering",
...
EI_NEW_NODES = [
    {
        ...
        "entry": False,
        "verify": "bench",
        "requires": ["atlas.ei.electronics_basics"],
        "source_refs": [
            {
                "relation": "adapted",
    ...
    },
    {
        ...
        "entry": False,
        "verify": "bench",
        "requires": ["atlas.ei.analog_circuits"],
        "source_refs": [
            {
                "relation": "adapted",
...
EI_NEW_EDGES = [
    {
        "from": "atlas.ei.electronics_basics",
        "to": "atlas.ei.digital_circuits",
        "relation": "requires",
    ...
    {
        "from": "atlas.ei.digital_circuits",
        "to": "atlas.ei.fpga",
        "relation": "requires",
    ...
    {
        "from": "atlas.ei.analog_circuits",
        "to": "atlas.ei.rf_circuits",
        "relation": "requires",
