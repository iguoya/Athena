...
> 后续：ADR 0024 已退出 WebView；本文背景中的 WebKitGTK / MD4C HTML 描述保留为
> 当时决策记录。现行运行时依赖以 `meson.build` 与 `scripts/package_linux.py` 为准
> （GTK、GtkSourceView、MD4C、SQLite），不再依赖 WebKitGTK。AppImage 基线由
> ADR 0019 调整为 Ubuntu 26.04。
...
- AppImage 由固定版本并校验 SHA-256 的 linuxdeploy 生成，作为 Ubuntu 基线及以上
  相近环境的便携下载选项。不承诺该 AppImage 在任意 Linux 发行版上免依赖运行。
...
- 未来若需跨发行版、强隔离发布，应新增 Flatpak 等方案，而不是把当前 AppImage
  描述成通用静态包。
