...
本应用是 `apps/` 下的平级学习应用之一（ADR 0045），没有任何特权：仓库根既没有它的
...
- **教学实验有两条路径，并存而不是二选一（ADR 0053）**：
  ...
  一个知识点可以两者都挂。**可实操的知识点默认每个配一个独立的骨架案例，一对一**
  ——不是挑几个有代表性的做，也不是几个知识点共用一个；只有确实无法用可运行代码
  观察的（纯概念辨析）才没有。理由是**读懂不等于写得出，而这个应用教的就是 C++
  本身**；共用案例会让某个知识点的动手环节退化成「顺带看了一眼」。
...
- **本应用只负责 C++ 语言这一个领域，不替别的领域搭知识框架。** 设计模式、数据结构
  ...
  应用，彼此平级）。它们在本应用里的唯一合法形态是**首页学科图谱上的一个节点**
  ——那是学科导航，说明它学什么、为什么值得学、和 C++ 是什么依赖关系，点击时按
  `app_id` 启动那个独立应用（ADR 0032 第 5 条）。
  **不得**在 `athena.json` 里为别的领域开分类、列章节。历史上开过两次，两次都长成
  了空目录表：`da`（数据结构与算法）23 章里 22 章没有知识点，`b534cfb` 删除，内容
  由 `apps/dsa` 承载；`dp`（设计模式）23 章一个知识点都没有，素材转入
  `apps/design-patterns/`（该应用尚未开工，所以图谱节点回落为「规划中」）。
  空分类比没有更糟——它占着分类首页的位置，让人以为那里有内容。
  判断边界时看**教的是什么**而不是**用什么语言写**：`STLContainer` 讲 `unordered_map`
  的哈希桶与负载因子，属于 C++ 标准库语义，归本应用；手写一个哈希表归 `apps/dsa`。
