...
- `resources/athena.json` 是项目配置的**唯一数据源**，当前承载分类、章节、分组和知识点元数据。
...
- `article` 的共享内容管线是 Markdown → HTML + `article.css`；平台后端只负责显示同一份 HTML，不在 GTK 中自行维护完整 Markdown 排版器。
...
- 分类、章节、分组和子章节图标都来自 `athena.json`，C++ 不按名称手写图标映射。
...
3. **数据驱动**：以 `athena.json` 为单一数据源，不把元数据硬编码进 C++。
