...
编写或生成学习文档时，应先读取章节和知识点的 `description`，围绕学习 C++ 所需的
概念、语义、边界和误区组织正文，并经人工审核。实现代码章节时，再以这些学习目标和
文档为依据，使用骨架编写可观察、可重复的辅助实验并补充测试。理论内容写入分类自己的
`handbook_documents`，不创建 C++ 类、运行按钮或注册项。

二者共同形成“文档建立思想和预期 → 代码实验产生行动和结果 → 对照结果修正并深化文档
理解”的循环。实验不是文档的附属截图，文档也不是代码的事后注释；两者围绕同一个 C++
知识点分工协作。
