...
- `SourceLocator` 按知识点成员函数名定位真实 C++ 定义范围。知识点条目本身（标题与描述）不响应点击；行尾操作区以分隔线隔离，集中放置“运行”按钮与“理解”“掌握”两个两态标记按钮（状态以位标志即时持久化，bit0 已理解、bit1 已掌握；两者齐备后运行按钮置灰，取消任一标记即可恢复运行）。运行结果上方依次是“历史”“解释”按钮行与“笔记”分组框，作用于当前激活的知识点：笔记编辑区固定约三行高度，停止输入后自动保存，“历史”查看运行记录，“解释”把说明与源码组成提示词放入剪贴板并唤起本机 AI 助手（默认 `doubao://`，可用环境变量 `ATHENA_AI_COMMAND` 自定义命令）。章节打开时默认无激活条目，状态栏保持占位提示并支持换行；源码面板使用带标题的 Frame（GroupBox 形态）组合标题与源码框。
