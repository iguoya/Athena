...
- “AI 自测”“AI 讲解差异”共用同一条 AI 调用链：`call_ai_chat_with_fallback(ark_api_key, deepseek_api_key, prompt)` 优先用火山方舟豆包（`doubao-seed-2-1-pro-260628`，环境变量 `ATHENA_ARK_API_KEY`），未配置或请求失败再退回 DeepSeek（`deepseek-chat`，环境变量 `ATHENA_DEEPSEEK_API_KEY`）；两者都未配置时“AI 自测”直接提示需要先配置至少一个 Key，没有剪贴板退路。这条优先级顺序反复过一次：最初豆包优先，中途以“豆包出题明显更慢”为由改成 DeepSeek 优先，现在改回豆包优先，速度差异仍在但不再作为排序依据。两家服务商都是 OpenAI 兼容协议，底层共用 `call_llm_chat(endpoint, model, api_key, prompt)`，只是 endpoint/model 不同，不是两套 curl 调用逻辑。Key 与请求体经临时文件传入、用后即删，不出现在进程参数里；网络请求在独立线程执行，结果经主线程回填（参见 ADR 0009、ADR 0010）。
