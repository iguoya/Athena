...
- “运行历史”打开运行记录对话框：左侧是最近运行列表（时间、耗时、与当前源码是否一致、运行时的 git 提交短哈希，工作区有未提交改动时加 `+`），右侧最多同时选中 2 条记录并排对比，每条记录一栏，栏内上方是运行时的源码快照（只读 GtkSourceView，C++ 语法高亮，不做逐行 diff）并在标题带完整 git 版本描述，下方是对应输出，默认选中最近两次运行；git 信息由运行时同步查询 `ATHENA_SOURCE_ROOT` 所在仓库得到，不在 git 仓库或 git 不可用时静默留空，不影响运行。配置了 `ATHENA_DEEPSEEK_API_KEY` 时对话框内另有“AI 讲解差异”按钮（选中恰好 2 条才可用，未配置 Key 时这个按钮不出现），把两条记录的源码快照与输出一并发给 DeepSeek，请求解释改动和结果变化的关系；不做逐行 diff 高亮，这部分交给 git 自己的工具。
- “AI 讲解”“AI 自测”“本章总纲”配置了环境变量 `ATHENA_DEEPSEEK_API_KEY` 时才走 DeepSeek，未配置时退回原有的剪贴板方案（“AI 讲解”“本章总纲”复制提示词到剪贴板并唤起本机 AI 助手，默认 `doubao://`，可用环境变量 `ATHENA_AI_COMMAND` 自定义命令；“AI 自测”直接提示需要先配置 Key，没有剪贴板退路）。三者都经 `call_deepseek_chat()` 走 curl 子进程发起请求，Key 与请求体经临时文件传入、用后即删，不出现在进程参数里；网络请求在独立线程执行，结果经主线程回填（参见 ADR 0009）。
- “AI 讲解”“AI 讲解差异”“本章总纲”共用 `show_ai_markdown_dialog()`：把 DeepSeek 的回复当 Markdown 解析，用跟 article 章节（如“程序与源码组织”）完全一样的排版显示——md4c 转 HTML、WKWebView（macOS）渲染，标题、列表、代码块都有正常版式；DeepSeek 的回答经常代码和说明夹杂，早期用纯文本 TextView 展示对代码不友好，改成这个之后代码块能正常保留缩进和等宽字体，不再是纯文本堆一坨。这几个对话框用的 `article.css` 在加载前追加了一段 `:root { --article-font-size: 22px; }` 覆盖，只影响这几个对话框，不改 `resources/article.css` 本身、不影响真正的 article 章节阅读页面（仍是原来的 19px）。“本章总纲”的提示词要求 DeepSeek 突出重点、不逐字复述知识点说明、不要开场白结束语，覆盖该章节全部知识点。
- “AI 自测”要求 DeepSeek 以 JSON 返回针对该知识点具体源码的自测题（题干、选项数组、正确选项下标数组 `correct_indices`、解释），继续用 GTK 控件（CheckButton + Label）渲染，不是 Markdown/WebView：题目数量、每题选项数量都不固定，由 DeepSeek 按该知识点实际包含的独立考察点客观决定，覆盖全部关键行为和易错点、不为凑数出太简单或重复的题；`correct_indices` 只有一个元素时按单选渲染（选项互斥），多个元素时按多选渲染（选项互相独立、可多选，标题标注“多选”），选完点“提交答案”才判对错——多选要求选中集合与正确答案集合完全一致才算对，不给部分分；正确显示绿色“✓ 回答正确”，错误显示红色“✗ 回答错误，正确答案是……”并展开解释，随后选项和提交按钮置灰。解析前先用 `strip_markdown_code_fence()` 去掉 DeepSeek 有时会加的 ` ```json ` 代码围栏，仍失败时退化为原样显示文本；正文字号用 `.ai-dialog-question`/`.ai-dialog-option` 等 class（22pt/20pt/18pt 一档），不影响主界面的 `.code-view`（18pt）。
- 对话框不使用 GTK 内建的 action area（默认靠右且样式不好单独控制），统一用 `append_dialog_action_bar()` 把按钮手工加在内容区末尾、居中；运行历史对话框里“AI 讲解差异”（紫色 `btn-ai-accent`，未配置 Key 时不出现）跟“关闭”分组放在一起。运行历史对比的源码/输出仍是 GTK TextView/GtkSourceView（`.ai-dialog-text`，22pt），不是 Markdown/WebView——那里展示的是原始运行记录，不是 AI 生成的说明文字。
