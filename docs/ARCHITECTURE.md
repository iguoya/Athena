...
- `SourceLocator` 按知识点成员函数名定位真实 C++ 定义范围。知识点条目本身（标题与描述）不响应点击；行尾操作区以分隔线隔离，集中放置“运行”按钮与两排独立的五星评分（重要度、熟练度，各 0–5 星，自由打分并即时持久化；再点当前星级则降一星；星星右侧带 2 字文字标识随星级变化，悬浮单颗星显示该星级含义；重要度着橙色，熟练度着绿色）：重要度供用户自行标注难度/权重，不影响运行按钮；熟练度到 5 星后运行按钮置灰，降低星级即可恢复运行。运行结果上方依次是“历史”“解释”按钮行与“笔记”分组框，作用于当前激活的知识点：笔记编辑区固定约两行高度，停止输入后自动保存，“历史”查看运行记录，“解释”把说明与源码组成提示词放入剪贴板并唤起本机 AI 助手（默认 `doubao://`，可用环境变量 `ATHENA_AI_COMMAND` 自定义命令）。章节打开时默认无激活条目，状态栏保持占位提示并支持换行；源码面板使用带标题的 Frame（GroupBox 形态）组合标题与源码框。
