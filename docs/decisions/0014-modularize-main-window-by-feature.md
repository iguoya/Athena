...
> 后续：ADR 0023 已移除运行历史及其写入链路；本文保留当时拆分
> `MainWindow` 的背景与模块边界记录，不代表该功能仍然存在。
...
本次改造按低风险到高风险逐步完成，每一步都保持应用可构建、可运行：
...
5. 最后提取状态最多的 `CodeChapterPage` 与 `ExperimentRunner`，统一知识点激活、源码
   高亮、运行、输出和持久化回调。
6. 删除迁移后留在 `MainWindow` 的转发方法和无效状态；窗口只保留顶层协调。
7. 将后续新增的有状态魔方实践页和静态关于对话框一并收进独立页面/对话框模块，
   防止特殊页面再次把内部控件树堆回窗口。
...
- 不以行数作为唯一目标；如果拆分后出现跨模块双向调用或共享控件指针，说明边界错误，
  不能仅为缩短文件继续拆类。
- 本 ADR 的全部阶段已经落地。`MainWindow` 只保留分类/标签导航、页面懒加载、跨页
  跳转、进度刷新和模块生命周期；页面内部控件树、WebView、后台线程、运行历史写入
  与对话框布局不再由窗口拥有。
- 后续细化：章节标签条与 Stack 子页装配抽成 `ChapterNavStrip`；本 ADR 里作为单个
  叶子模块的 `LearningDialogs` 按单一职责进一步拆成门面 + 子对话框，见 ADR 0016。
