...
> 版本缠斗」。钉版本那一步已回滚。
>
> **连 gtkmm 一起降到 4.18 也试了，同样失败，而且失败点一字不差。** 三个包都降
> 到位（glib 2.88.3 / glibmm-2.68 2.86.0-4 / gtkmm-4.0 4.18.0-3），报的还是
> `gio-2.0 >= 2.89.3 could not be satisfied`。这就定位到了最后一层：
> **那个下限来自 glibmm 包自己的 `giomm-2.68.pc`，不是 gtkmm 带来的。**
>
> 于是 MSYS2 的 `glibmm-2.68 2.86.0-4` 处在一个自相矛盾的状态：
>
> - 它的 `.pc` 要求 **glib >= 2.89.3**（说明这个包是用 2.89.3+ 构建的）；
> - 它的**源码是 2.86**，而适配 glib 2.89.2 final type 改动的是 glibmm 2.89.0。
>
> 换句话说，这个包被打成了「要求新 glib，却没有新 glib 需要的代码」。用 glib
> 2.90 则头文件冲突，降 glib 则它自己的 .pc 不满足——**两头都堵是打包缺陷造成
> 的，不是我们选错了版本组合**。理论上还能再往下试 `2.86.0-1/-2/-3` 这些更早的
> 版次，但那已经是在逐个 revision 试错，正是本 ADR 说的「跟上游版本缠斗」。
> 降级尝试全部回滚。
>
