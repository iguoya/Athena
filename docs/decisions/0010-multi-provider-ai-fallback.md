# ADR 0010：AI 讲解/自测新增火山方舟豆包服务商，按优先级回退到 DeepSeek
...
ADR 0009 让"AI 讲解"等功能可选接入 DeepSeek，并且已经预见到"如果未来要
...
- 网络请求路径仍然只有 curl 子进程一条，没有新增生产依赖，符合 ADR 0009
  的既有约束。
...
- "本章总纲"仍然是纯本地静态文档（见 `overview_document`
  字段），不参与这条 AI 调用链——这是 ADR 0009 之后又做的单独修正，两者
  不冲突。
