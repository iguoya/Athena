...
正文按 ADR 0028 第 6 节的**八个固定字段**组织，每章体例一致：

| 字段 | 回答什么 |
|---|---|
| 痛点 | 没有它之前，现实工程里出什么事 |
| 来历 | 何时、为解决什么被引入；后来怎么演进 |
| 意图 | 一句话：它承诺解决什么 |
| 基本问题 | 贯穿全章、要反复自问的 2–4 个开放问题 |
| 心智模型 | 用什么结构或比喻去理解它 |
| 判断点 | 实际写代码时它让你做哪些选择 |
| 边界与代价 | 何时不该用、用了付什么代价 |
| 落点 | 它是后面哪些知识的地基 |

八个字段都要出现，顺序可调、允许合并进同一节，但不得整段缺失；末尾仍保留
...
标题旁，用户不能修改；与通过完整 AI 自测成绩自动换算的“熟练度”是两个独立概念，
后者存在本地数据库中且不能手动修改，`importance` 只存在 `athena.json` 里。缺省为
`0`（未评）。
