...
- **每章第一个标签是「本章导览」，第二个才是「教学大纲」（ADR 0036）**。导览是极简
  概要，回答"这一章有什么、怎么串起来、**哪个重哪个难**"，**只给结论不给理由**
  ——不写为什么这样分、不写评定理由、不举例、不写 API、不写易错点（那是「小结」的事）。
  **难度与掌握目标必须出现在导览里**：它是地图，决定学习投入的恰恰是轻重缓急；省掉的
  是"为什么是这个难度"，不是"难度几档"。知识点清单按掌握目标分档组织，分档顺序就是
  推荐顺序。
  **以图为主**：概念之间怎么串、哪节为哪节做铺垫，画出来比列出来快得多；能由
  `requires` 算出来的结构就画成活的并让节点可点跳转。**简洁不等于纯文字**——删的是
  解释性文字，不是图。导览里的说法必须与大纲同源，不得给出大纲之外的新判断（比如
  大纲没排过的顺序、没评过的难度），否则就是把 ADR 0034 刚扔掉的"两份内容会漂移"
  换个名字请回来。大纲**不因此变短**，它继续承担解释责任。
...
- **回顾与自查的职责要落实，但形式不固定——拒绝为凑格式而写**。读者学完一节，
  ...
  考核、误区清单，或这一节本来就有的收尾段落。**一节已经有随堂考核时，不要再补
  一段小结**：考核逐题判分并给解析，回顾自查已经落实，再写一段概括是形式化重复。
  判断标准是"有没有地方让读者回头确认"，不是"有没有一个叫「小结」的标题"。
  写小结时，用一到几段加必要的清单，概括讲了什么、要点与易错点、知识点之间的
  关系；它是给读者回顾用的，不是新知识。
