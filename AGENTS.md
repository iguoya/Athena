...
- MD4C（文章章节 Markdown 到结构化 `DocModel` 的解析）
- GTK `DocumentView`（文章目录、正文、字号和主题控制；不使用 WebView）
...
- Markdown 学习文档必须从“学习 C++ 知识点”的角度组织和生成，是知识体系、概念语义、适用边界、
...
- **Markdown 是参考资料，不是学习页面的布局协议。** 原生学习场景可自由使用 Blueprint、
  GTK 控件、图片/SVG、图表、流程图和合适的动态绘制来组织“预期—验证—反馈”；不得通过
  Markdown 的 H2/H3、段落或锚点反向限定场景顺序和表现形式。场景需要引用体系化解释时，
  提供跳转到分类手册的明确入口即可（ADR 0026）。
- 手册正文统一放在 `resources/articles/`，按分类收录进 `category.handbook_documents`（每个分类一部独立手册）；
  手册文档不生成课程类、成员函数、运行按钮或结果区，共享层解析为 `DocModel`，由跨平台 GTK
  `DocumentView` 显示。
  `chapter.overview_document` 指向本分类手册内一份已收录文档的起始锚点，不是单独展示的文档。
- **学习内容分三层，方向自上而下（ADR 0028）**：
  1. **教学大纲文档**（一章一份 Markdown，即 `chapter.overview_document` 指向的
     `*_overview.md`）**只指引大方向**，按固定范式组织：一句题注（承诺解决什么）加五节
...
- **优先图文结合，能用图说明的不要只堆文字**。概念结构、状态迁移、类型/对象关系、执行流程、
  ...
  - 首选**静态 SVG**（放 `resources/articles/images/` 下，Markdown 用 `![说明](images/xxx.svg)`
    引用），零脚本、`DocumentView` 原生显示、离线可用；也可用 Markdown 表格。原生学习场景
    也可直接使用同类资产，不必通过 Markdown 引用。
...
- 主程序按 `apps/*/app.json` 在首页生成入口并 spawn 进程；新增应用不改主程序代码。
...
- 跨平台能力同样受上述层次和依赖方向约束。当前支持 macOS 与 Ubuntu，相关改动遵守：
  ...
  - 学习内容的 `DocumentView` 必须在 macOS 与 Ubuntu 履行加载、锚点跳转、字号、
    主题和外部链接契约；不得以平台特有 WebView 作为内容渲染旁路。
