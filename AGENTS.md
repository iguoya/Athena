...
- **每章第一个标签是「简纲」，第二个才是「教学大纲」（ADR 0036）**。简纲是大纲的
  压缩视图，不是新内容：里面出现的每一条都必须是大纲里已有的标题或标签，**不得引入
  大纲没有的说法**，否则就是把 ADR 0034 刚扔掉的"两份内容会漂移"换个名字请回来。
  写法是**只留标签、删掉解释**：每条一行，不写理由、不举例、不写 API，全章一屏看完
  （1440 逻辑像素内不滚动，超了说明还在解释）。固定四块：一句题注、主脊（本章反复
  使用的判断动作的名字）、知识点清单（带难度档与掌握目标，无评定理由）、推荐顺序。
  每一条都要可点击跳到大纲或该知识点的教学过程——它是导航面，不只是摘要。
  简纲不写易错点（那是「小结」的事），小结不写难度评级。
