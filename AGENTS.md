...
- **日常打开哪个应用，走 `launcher/` 的菜单栏启动器**（ADR 0044）：它读仓库根
  `app.json`（主程序）和 `apps/*/app.json`，点一下就打开，已在跑的只前置窗口。
  主程序在那张列表里和各学科平级，不再是打开别的应用的必经之路；图谱入口保留。
  新增应用照样放一份 `app.json` 即可，启动器不需要改代码。
