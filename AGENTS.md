...
- 学习文档必须从“学习 C++ 知识点”的角度组织和生成，是知识体系、概念语义、适用边界、
  常见误区和思维模型的主要载体；不得按现有源码的文件结构、类结构或函数实现顺序写成
  源码说明书，也不得通过总结现有实现反向决定应教授哪些知识。
- 教学方向由知识点和学习文档主导，代码负责把思想转化为短小、真实、可运行且结果可观察
...
- 内容分为两条线：可实验知识点由章节的课程类和成员函数承载；理论、原则和工程思想由分类手册承载。
  两条线在配置里是并列的，不是同一种章节的两个变体——`content: article` 章节类型和 `chapter.document`
  字段已由 ADR 0012 废弃，现在所有章节都是同一种。
- 章节的核心闭环是“选择知识点 → 查看真实源码 → 运行对应成员函数 → 查看输出结果”。
- 一个课程类对应一个一级主题；一个 public 成员函数对应一个可独立运行的二级知识点。
- 手册正文统一放在 `resources/articles/`，按分类收录进 `category.handbook_documents`（每个分类一部独立手册）；
...
- 面向对象设计遵循以下六条原则；它们是上一条总纲在类粒度上的落地，冲突时以本文件
  ...
  - **里氏替换（LSP）**：任何接口（如 `ArticleView`）的实现必须完整履行其契约，
...
- 跨平台能力同样受上述层次和依赖方向约束。当前支持 macOS 与 Ubuntu，相关改动遵守：
  - 共享层、领域层、教学实现和页面业务逻辑不得包含 `__APPLE__`、Cocoa/WKWebView、
    ...
    已有抽象接口（例如 `ArticleView`）隔离。
  - Meson 必须按目标平台选择源码和系统依赖；不得为了 Linux 在共享代码里增加平台分支，
    也不得让 Linux 构建链接 `gtk4-macos`、Apple Framework 或 Objective-C++ 源文件。
  - macOS WKWebView 与 Ubuntu WebKitGTK 6.0 后端必须完整履行 `ArticleView` 的加载、
    锚点跳转、字号、主题和外部链接契约，不能只做“能显示”的残缺实现；未知或尚未支持的
    平台可以使用 GTK 文本回退，但不得把回退能力描述成完整平台支持。
