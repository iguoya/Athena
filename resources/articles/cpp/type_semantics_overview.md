...
下面六类风险按本章「判断与代价」的五个检查点排列，一条风险对应一条检查线，
后面遇到它们时可以直接回到对应的检查点。
...
- **修改了副本，而不是原对象。**（④ 辨表达式）写法很像，实际改变的对象却不同。
- **一次传递产生了不必要的复制。**（④ 辨表达式）对大对象而言，隐藏的复制会变成真实的
  时间和内存代价。
...
每个方向后面标着两项评级（ADR 0029）：前一项是**难度**，入门／简单／中等属于初中级，
进阶／高级属于高级内容；后一项是**掌握目标**——需要精通、必须掌握、一般了解。
两项互不推导：难的不等于可以跳过（值类别就是），简单的也不等于随便看看
（枚举边界很简单，却每次设计接口都要用）。标着“贯穿”的方向没有独立实验，
它在别的方向里反复出现。

**类型与约束**

- 静态类型如何表达意图、限制操作，以及编译器能够检查到哪里。〔贯穿 · 必须掌握〕
...
1. 指出一段代码里哪些名字指向同一个对象、哪些是各自独立的副本。〔需要精通〕
2. 说出一个对象从哪一行开始存在、到哪一行结束，并判断某个引用此刻是否还有效。
   〔需要精通〕
3. 面对一次初始化，说明它提供了什么保证、可能在哪里丢失信息、谁来检查。〔需要精通〕
4. 判断一个表达式是左值、纯右值还是将亡值，并据此说明它能绑定到哪种引用。〔必须掌握〕
5. 看到一次转换，说出它表达什么意图、检查发生在编译期还是运行期、哪些前提仍由自己
   保证。〔必须掌握〕
6. 面对“要副本还是要别名”的选择，说明依据：是否要改原对象、复制的代价、
   原对象的寿命够不够。〔需要精通〕
