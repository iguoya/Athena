...
> **这一章承诺解决什么**：把"对象怎么诞生、表达式指着什么、类型变化谁把关"
> 这三件事，变成读代码时就能判断、编译期就能检查的明确规则，而不是靠程序员的
> 记性和运行期的运气。

这是本章的教学大纲，按 ADR 0028 的四个字段组织：**痛点与来历 → 心智模型 →
判断与代价 → 落点**。它只讲方向，不讲语法机制，也不绑定练习或实验——那些由
学习页在这个框架内自行发挥。
...
要注意的是，它承诺的不是"更安全"或"更快"，而是**让意图在代码里显形**——你想要
副本还是别名、这次转换属于哪一类、这个值能不能被搬走，都要写出来，编译器才有
可能替你把关。
...
这个模型的用法，是把它变成四个反复自问的问题。它们贯穿全章，没有一劳永逸的
答案，每读一段陌生代码都要重问一遍：
...
之所以说"选择"而不是"正确写法"，是因为每一项能力都有代价。这一章教的是判断，
不是"永远用某种写法"：
